function [geompred,FVAL,EXITFLAG,OUTPUT,LAMBDA] = runknitro( sgseq, potential, sgbounds, infoneeded )
error('opti.runknitro is deprecated, use opti.KnitroProblem instead');
...
% gamma],[ns ng]));
fobj = @(x) opti.seq_pots(x,sgseq,potential);
...
for i = ind_c,    
    ...
    if (i>1 && i<=nfrags)
        frag_sgamma_i = sgseq(i-1);
      	frag_sgamma_ip = sgseq(i);
...
for i = ind_z,    
    frag_sgamma_i = sgseq(i-nfrags-1);
...
[geompred,FVAL,EXITFLAG,OUTPUT,LAMBDA] = ktrlink(fobj,X0,A,B,Aeq,Beq,LB,UB,NONLINCON);
