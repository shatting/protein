function [S,SV,CL,jsofeachs,endj,endi,pastis,CLcol,jeq1,L] = onesequenceforp(p,data,confus,class_info,newclasses,nums,nfound,CL,jsofeachs,endj,endi,past,CLcol,S,SV,jeq1,L)
...
if newclasses == 1
  [sb,gamma] = new_deal_hefg_interface(class_info,data{p}.bond,seq );
  clear sb;
else
  gamma = seq2gammaclasses(seq,class_info);
end
...
% store -log(confus) for less calculations. set confus = zero to infinity
minuslogconfus = zeros(size(confus));
for i = 1:size(confus,1) %should be = numgamma
    for j = 1:size(confus,2) %should be = nums
        if confus(i,j) ~= 0
            minuslogconfus(i,j) = -log(confus(i,j));
        else
            minuslogconfus(i,j) = inf;
        end
    end
end
...
if newclasses == 0
    ...
    feasiblepairmatrix = ones(nums); % 4 bc 4 classes
end
...
[Vmin,Vminf,Rk,Lj,mins] = findpartialsumsforp(p,confus,class_info,data,newclasses);
s1 = mins(:,1);
clear Lj;
...
if newsequences == 0
    ...
    if sno == 1
        for s = 2:nums
            if newclasses == 0
                ...
                A1(s) = minuslogconfus(gamma(1),s) + Rk(Rkindex,2);
            else
                A1(s) = minuslogconfus(gamma(1),s) + Rkmin(2);
            end
    ...
    % so, now I have to do the same thing for i = 2:n. it's trickier now, bc
    ...
    for i = 2:n-1
        ...
        sumb4 = sumb4 + minuslogconfus(gamma(i-1),s1(i-1));
            ...
            for s = 2:nums
                ...
                if feas == 1 % if sminu --> s possible
                    if newclasses == 0
                        cpi = tentoXconverter(s,2); %a(1); cpi = a(2); 
                        Rkindex = cpi; %minimum for s_(j+1).c = cpi   
                       A(s) = sumb4 + minuslogconfus(gamma(i),s) + Rk(Rkindex,i+1);
                    else
                        A(s) = sumb4 + minuslogconfus(gamma(i),s) + Rkmin(i+1);
                    end
                end
            end
        elseif ((1 < sno) && (sno < nums))
    ...
    sumb4 = sumb4 + minuslogconfus(gamma(i-1),s1(i-1));
        ...
        if sno == 1
            for s = 2:nums
                feas = feasiblepairmatrix(sminus,s);  %the new s_(i) has to be compatible with s1_(i-1)
                if feas == 1 % if sminu --> s possible
                       A(s) = sumb4 + minuslogconfus(gamma(i),s);
                end
            end
        elseif ((1 < sno) && (sno < nums))
            for s = 1:(sno-1)
    ...
    for i = 1:ind-1
        S(2,i) = S(1,i);
        sumb4 =  sumb4 + minuslogconfus(gamma(i),S(2,i));
    end
    ...
    for s = 1:nums
        ...
        if feas == 1
            ...
            if ind < n
               psum = sumb4 + minuslogconfus(gamma(ind),s) + Rk(Rkindex,ind+1);
            else
                psum = sumb4 + minuslogconfus(gamma(ind),s);
            end
            if psum < psummin  
                psummin = psum;
                sind = s;
    ...
    % check! 
    %psummin, pot2
    sumb4 =  sumb4 + minuslogconfus(gamma(ind),S(2,ind));
    ...
    for i = ind+1:n
       ...
       for s = 1:nums
          ...
          if feas == 1
            ...
            if i < n
               psum = sumb4 + minuslogconfus(gamma(i),s) + Rk(Rkindex,i+1);
            else
                psum = sumb4 + minuslogconfus(gamma(i),s);
            end
            if psum < psummin  
                psummin = psum;
                si = s;
    ...
    %usedsjs = cell(n,nfound); % to keep track of sj's used coming from a certain position in CL
    %usedsjs{j,i } = S(1,j);
...
for L = a:nfound  %the index of the sequence we are looking for: s^{L}
    ...
    % need to find the minimal potential of D_{r} for r = j,...n (D_{r} =
    % inf, for r = 1,..,j-1
    ...
    sumb4 = 0;
    for k = 1:j-1
        sumb4 = sumb4 + minuslogconfus(gamma(k),S(L-1,k));
    end
    sumfirst = sumb4;
...
%         end
       ...
       % end
        ...
        if feas == 1
            ...
            if j < n
                VDjs(s) = sumb4 + minuslogconfus(gamma(j),s) + Rk(Rkindex,j+1);
            else 
                VDjs(s) = sumb4 + minuslogconfus(gamma(j),s);
            end
%             if VDj < minVDj
%                 minVDj = VDj;
%             end
    ...
    for k = j+1:n
        ...
        sumb4 = sumb4 + minuslogconfus(gamma(k-1),S(L-1,k-1));
        ...
        for s = 1:nums
            ...
            if feas == 1
                ...
                if k < n
                    Vdks(s) = sumb4 + minuslogconfus(gamma(k),s) + Rk(Rkindex,k+1);
                else
                    Vdks(s) = sumb4 + minuslogconfus(gamma(k),s);
                end
               % if VDk < minVDk
               %     minVDk = VDk;
               % end
...
%      end
     ...
     for k = 1:j-1
         sumb4 = sumb4 + minuslogconfus(gamma(k),S(L,k));
     end
...
%         end
        ...
        if feas == 1
            ...
            if j < n
               psum = sumb4 + minuslogconfus(gamma(j),s) + Rk(Rkindex,j+1);
            else
                psum = sumb4 + minuslogconfus(gamma(j),s);
            end
    ...
    S(L,j) = sj;
    % update list of sj's coming from C(j,i) - make a cell of size (n,nfound),
    % and in each entry have a list of sj's used!
    jsofeachs(L-1) = j;
    
    sumb4 =  sumb4 + minuslogconfus(gamma(j),S(L,j));
    
    % check!
    
    %now find the rest of the s_{k}'s (k = j+1 : n)
    for k = j+1:n
       psummin = inf;
       ...
       for s = 1:nums
          ...
          if feas == 1
            ...
            if k < n
               psum = sumb4 + minuslogconfus(gamma(k),s) + Rk(Rkindex,k+1);
            else
                psum = sumb4 + minuslogconfus(gamma(k),s);
            end
            %if psum < psummin  
            %    psummin = psum;
            if (psum - potn) < 10^(-10)
...
for k = 1:nfound
  for l = 1:nfound
    A(k,l) = percentsequenceidentity(S(k,:),S(l,:));
    if A(k,l) > .999999
        if k ~= l
          k
          l
