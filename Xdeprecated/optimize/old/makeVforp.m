function [Vr,Vl] = makeVforp(seq,confus,class_infos,p,newclasses)
% seq = data{p}.seq 
...
if newclasses == 0
    gamma = seq2gammaclasses(seq,class_infos);
    n = size(gamma,2); %technically n - 3, but whatever

else
    load RT071127.mat  %I really should be able to find gammas without bonds!!!
    [sb,gamma] = new_deal_hefg_interface(class_infos,data{p}.bond,seq );
    clear sb;  
    n = size(gamma,1); %technically n - 3, but whatever
