function [ optipot ] = optipotential_make( usehefgclasses, classdata_name, problemname )
%OPTIPOTENTIAL_MAKE generates optimization parameters, that depend on the
%gamma and s classification only, but not on the protein to be predicted
...
classdata = classdata_load(classdata_name); % new_deal_info
...
if ~usehefgclasses  
  ...
  HEL27_getfeats; % HEL27feats, hel27, fragaaseq
  
  all_s = hel27;
  all_gamma = double(classdata.final.cl(:,end));
  
  z = HEL27_transform2z(HEL27feats,all_s);
  optifeats = [HEL27feats(:,1:2), z];
  dprintf('generating s/gamma potentials');
...
optipot = struct;
optipot.optifeats = optifeats;
optipot.all_s = all_s;
optipot.all_gamma = all_gamma;
optipot.potential = potential;
optipot.confus = confus;
optipot.sgbounds = sgbounds;
optipot.usehefgclasses = usehefgclasses;
optipot.classdata = classdata;
optipot.problemname = problemname;
