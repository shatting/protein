...
while 1,
  ...
  for c=1:ncl,
    ...
    pot.mean(:,c)=sum(cdata,1); % not yet mean but stat
    pot.cov{c}=cdata'*cdata;    % not yet cov but stat
  end;
  ...
  covclass_test_plot(iteration, data, cl, pot);
  showtime(toc-tim)
  disp(' ');
  if ndif==0 || ndif>=max(level),       
      break;
  end;
