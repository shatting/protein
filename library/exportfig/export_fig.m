...
%   -cmyk - optional string indicating that color figures should be saved
%           in the CMYK colorspace rather than the RGB colorspace
%           (default). Only supported in pdf, eps and tiff output.
...
if isvector(options)
    ...
    try
        ...
        % Generate a pdf
        eps2pdf(tmp_nam, pdf_nam, options.crop, options.append);
    catch
        % Delete the eps
        delete(tmp_nam);
        rethrow(lasterror);
